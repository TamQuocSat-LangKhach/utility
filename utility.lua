...
---@param NCards integer[]|integer @ 待翻开的卡牌，为数字的场合，正数从牌堆顶翻开，负数从牌堆底翻开
---@param skillName string @ 技能名
...
Utility.turnOverCardsFromDrawPile = function(player, NCards, skillName, visibleData)
  ...
  if type(visibleData) == "boolean" then
    if visibleData == false then
      visiblePlayers = player.id
    end
  else
    visiblePlayers = visibleData
  end
  if type(NCards) == "table" then
    room:moveCards {
      ids = NCards,
      toArea = Card.Processing,
      moveReason = fk.ReasonJustMove,
      skillName = skillName,
      proposer = player.id,
      moveVisible = (visibleData == nil or visibleData == true),
      visiblePlayers = visiblePlayers,
    }
    return NCards
  else
    local to_show = room:getNCards(math.abs(NCards), NCards > 0 and "top" or "bottom")
    room:moveCards {
      ids = to_show,
      toArea = Card.Processing,
      moveReason = fk.ReasonJustMove,
...
Utility.returnCardsToDrawPile = function(player, cards, skillName, toTop, visibleData)
  ...
  room:moveCards {
    ...
    moveVisible = (visibleData == nil or visibleData == true),
    visiblePlayers = visiblePlayers,
    drawPilePosition = toTop and 1 or -1
  }
end
...
Fk:addTargetTip{
  ...
  target_tip = function(_, to_select, _, _, _, _, extra_data)
    if not table.contains(extra_data.targets, to_select) then return end
